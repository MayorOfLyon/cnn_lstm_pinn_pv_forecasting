...
def data_preprocessing(path, train_list_i):

    train_old = pd.read_csv(path + train_list_i, encoding='GB18030')
    #重新划分数据集将前4/5的数据划分为训练集，后1/5的数据划分为测试集
    train_old, test_old = train_old.iloc[:int(len(train_old)*0.8),:], train_old.iloc[int(len(train_old)*0.8):, :]
    test_old = test_old.reset_index(drop=True)
    
    train_old['时间'] = pd.to_datetime(train_old['时间'], format='%Y-%m-%d %H:%M')
    test_old['时间'] = pd.to_datetime(test_old['时间'], format='%Y-%m-%d %H:%M')
    
    # train_old['year'] = train_old['时间'].apply(lambda x: x[0 : 4]).astype('int32')
    # train_old['month'] = train_old['时间'].apply(lambda x: get_month(x, train_list_i)).astype('int32')
    # train_old['day'] = train_old['时间'].apply(lambda x: get_day(x, train_list_i)).astype('int32')
